...
export function IntegrationChecklist({
    ...
    const runChecks = () => {
        ...
        const checks: ChecklistItem[] = [
            // Core Components
            {
                ...
                details:
                    currentDigit !== null
                        ? 'Component loaded with current digit: ' + currentDigit
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case 'fail':
                return (
                    <Badge className='bg-red-500/20 text-red-400 border-red-500/30 hover:bg-red-500/30'>Failed</Badge>
                );
            case 'warning':
                return (
            ...
            <Card className={theme === 'dark' ? 'bg-[#0f1629]/80 border-blue-500/20' : 'bg-white border-gray-200'}>
                ...
                <CardContent>
                    <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-6'>
                        ...
                        >
                            <div
                                className={`text-3xl font-bold ${theme === 'dark' ? 'text-yellow-400' : 'text-yellow-600'}`}
                            >
                                {counts.warning}
                            </div>
                            <div className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                        ...
                        >
                            <div
                                className={`text-3xl font-bold ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}
                            >
                                {counts.pending}
                            </div>
                            <div className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
