...
export const StatPill: React.FC<StatPillProps> = ({ label, value, variant = 'default', suffix, icon }) => {
    const valueClass = {
        default: 'stat-value',
...
export const InputField: React.FC<InputFieldProps> = ({
    ...
    <div className='w-full'>
        ...
        <div className='relative flex items-center'>
            {prefix && <span className='absolute left-3 text-text-tertiary font-medium'>{prefix}</span>}
            <input
                type={type}
                value={value}
            ...
            />
            {suffix && <span className='absolute right-3 text-text-tertiary font-medium'>{suffix}</span>}
        </div>
    </div>
);
...
export const GlowBadge: React.FC<GlowBadgeProps> = ({ children, color = 'blue', size = 'md' }) => {
    const colorClasses = {
    ...
    return (
        <div className={`${colorClasses[color]} ${sizeClasses[size]} border rounded-full font-semibold inline-block`}>
            {children}
        </div>
    );
};
...
export const LoadingSpinner: React.FC<{ size?: 'sm' | 'md' | 'lg' }> = ({ size = 'md' }) => {
    ...
    return <div className={`${sizeClass} border-4 border-transparent border-t-blue-500 rounded-full animate-spin`} />;
};
...
export const StatGrid: React.FC<StatGridProps> = ({ items, columns = 3 }) => {
    ...
    return (
        <div className={`grid ${gridClass} gap-4 w-full`}>
            {items.map((item, idx) => (
                <StatPill key={idx} label={item.label} value={item.value} variant={item.variant} suffix={item.suffix} />
            ))}
        </div>
    );
};
...
export const TradeCard: React.FC<TradeCardProps> = ({ digit, amount, result, time, bot }) => {
    const resultColor = {
